...
if __name__ == "__main__":
    load_model= modell(n_features=n_features,model_path='./A_999.pth')
    datasets, y_true=dataprocessing(csv=csv,split_date='2018/12/24', mode= 'test')
    for i in range(0,15,3):
        start =i     
        end = start+previous_days
        last = end + predict_day
        y_pred_inv , dataset_test_y , past_data = prep_data(load_model,datasets, y_true , start , end , last) 
